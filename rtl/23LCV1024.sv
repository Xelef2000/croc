// *******************************************************************************************************
// **                                                                                                   **
// **   23LCV1024.v - 23LCV1024 1 MBIT SPI SERIAL SRAM  WITH BATTERY BACKUP AND SDI INTERFACE           **
// **                                                                  (VCC = +2.5V TO +5.5V)           **
// **                                                                                                   **
// *******************************************************************************************************
// **                                                                                                   **
...
// **   11/03/2014:  Modified LC version for LCV SPEC                                                   **
// **                                                                                                   **
// *******************************************************************************************************
// **                                       TABLE OF CONTENTS                                           **
// *******************************************************************************************************
...
module M23LCV1024 (SI_SIO0, SO_SIO1, SCK, CS_N, VCC, VBAT, RESET);  
   ...
   input                VCC;                            // VCC is modeled as digital for simulation purposes
    
   input                VBAT;                           // VBAT is modeled as digital for simulation purposes
...
// *******************************************************************************************************
   ...
   wire                 PowerGood;                      // power status from either source
...
// -------------------------------------------------------------------------------------------------------
   ...
   always @(negedge VCC) begin
      if (VBAT == 0) begin
         for (i = 0; i < 131071; i = i + 1)
            MemoryBlock[i] <= 8'bx;     // or use = if MemoryBlock is not a reg
   ...
   begin
      if (VCC == 0)
       for (i=0; i<131071 ;i=i+1) MemoryBlock[i]=8'bx; 
   end 
...
// -------------------------------------------------------------------------------------------------------
   ...
   always @(posedge SCK) begin
      if ((PowerGood == 1) & (InstructionREAD | InstructionWRITE)) begin
         case (IOMode)
            `SPIMODE: begin
...
// -------------------------------------------------------------------------------------------------------
   ...
   always @(posedge SCK) begin
      if ((PowerGood == 1) & InstructionWRMR) begin
         case (IOMode)
            `SPIMODE: begin
      ...
      case (IOMode)
         ...
         `SDIMODE: begin
            if (ClockCounter == 3) begin
              if ({DataShifterI[05:00],SO_SIO1,SI_SIO0} == `RSTIO) IOMode <= `SPIMODE;
            end
         end
...
// -------------------------------------------------------------------------------------------------------
   ...
   always @(posedge SCK) begin
      if (PowerGood == 1 & InstructionWRITE) begin
         case (IOMode)
            `SPIMODE: begin
...
// -------------------------------------------------------------------------------------------------------
   ...
   // Replace timing with logic (you lose analog delay, which Verilator doesn't simulate anyway)
   assign OutputEnable1 = SO_Enable;       // tV delay ignored
   assign OutputEnable2 = ~CS_N;           // tDIS delay ignored
   assign OutputEnable3 = PowerGood;       // straight logic
   ...
   specify
      `ifdef TEMP_INDUSTRIAL
         specparam
            ...
            tVCChi = 25,                                // Interpretation of vcc/vbat slews
            tVCClo = 100,
            tVBAThi= 25,
            tVBATlo= 100; 
      `else
